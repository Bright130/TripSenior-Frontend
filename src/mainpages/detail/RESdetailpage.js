...
export default class RESDetailpage extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    loadAPI(
      " http://127.0.0.1:5000/res?place_id=" + this.props.match.params.id
    ).then(data => {
      this.setState({
        placeInfo: data,
        img: [
        ...
        <div className="detailpage-1">
          ...
          <div className="detailpage-detail_instance-6">
            <Detail
              ...
              opentime={this.state.placeInfo["time"]}
              phone={this.state.placeInfo["phone"]}
              fee={this.state.placeInfo["price"]}
